...
def parse_model(d, ch, verbose=True):
    ...
    stage_repeat_modules = frozenset({FusedMBConv, MBConv, NAM_MBConv})
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["neck"] + d["head"]):  # from, number, module, args
        ...
        if n > 1 and m in stage_repeat_modules:
            if m is FusedMBConv:
                m_ = EV2Stage(FusedMBConv, c1, c2, n=n, s=args[-1], expansion=args[2], se_ratio=args[3])
            elif m is MBConv:
                m_ = EV2Stage(MBConv, c1, c2, n=n, s=args[-1], expansion=args[2], se_ratio=args[3])
            elif m is NAM_MBConv:
                m_ = EV2Stage(NAM_MBConv, c1, c2, n=n, s=args[-1], expansion=args[2])
            else:
        ...
        print(f"Layer {i} | from={f} | module={m.__name__} | c2={c2}")
        print("i =", i, "ch =", ch)
        layers.append(m_)
        if i == 0:
            ch = []
